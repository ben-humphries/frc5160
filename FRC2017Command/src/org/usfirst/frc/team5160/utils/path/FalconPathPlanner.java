 ...
 *
 */
public class FalconPathPlanner implements Path
{
	...
	private Map<Double, Object> timeMap = new TreeMap<Double, Object>();
	 ...
	 * @param path
	 */
	public FalconPathPlanner(Waypoint[] path)
	{
		waypoints = path;
		origPath = new double[waypoints.length][2];
		for(int i = 0; i < waypoints.length; i++){
			origPath[i] = new double[]{waypoints[i].x, waypoints[i].y};
		}
	...
	public SidePath[] compile(double totalTime, double width){
		this.calculate(totalTime, TIMESTEP, width);
		...
		for (int i = 0; i < smoothPath.length; i++){
			double angle = Math.toRadians(heading[i][1]);
 ...
 */
	...
	public static void main(String[] args){
		Waypoint[] waypoints = {
				new Waypoint(8, 8, 4 , -4, 0),
				new Waypoint(7, 7, 0 , -5, 0),
